...
CREATE TABLE `sys_crit_log` (
                                ...
                                KEY `idx_saas_request_date` (`saas_id`,`request_date`)
...
CREATE TABLE `sys_data_history` (
                                    ...
                                    KEY `idx_saas_create_date` (`saas_id`,`create_date`)
...
CREATE TABLE `task_croner_info` (
                                    ...
                                    `task_desc` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci COMMENT '任务描述',
                                    ...
                                    `alert_wait_timeout` int DEFAULT NULL COMMENT '等待超时(毫秒)',
                                    `alert_run_timeout` int DEFAULT NULL COMMENT '运行超时(毫秒)',
...
-- uw_task.task_delayer_info definition

CREATE TABLE `task_delayer_info` (
                                     `id` bigint NOT NULL COMMENT 'id',
                                     `task_name` varchar(200) DEFAULT NULL COMMENT '任务名称',
                                     `task_desc` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci COMMENT '任务描述',
                                     `task_class` varchar(200) DEFAULT NULL COMMENT '执行类信息',
                                     `task_owner` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '任务所有人',
                                     `task_tag` varchar(100) DEFAULT NULL COMMENT '运行标签',
                                     `run_target` varchar(100) DEFAULT NULL COMMENT '运行目标',
                                     `consumer_num` int DEFAULT '1' COMMENT '执行线程数',
                                     `poll_interval` bigint DEFAULT '3' COMMENT 'poll间隔秒',
                                     `prefetch_num` int DEFAULT '5' COMMENT '单次poll最大条数',
                                     `rate_limit_type` int DEFAULT '0' COMMENT '限速类型',
                                     `rate_limit_value` int DEFAULT '10' COMMENT '限速窗口配额上限',
                                     `rate_limit_time` int DEFAULT '1' COMMENT '限速窗口长度(秒)',
                                     `rate_limit_wait` int DEFAULT '30' COMMENT '限速不足时等待秒数',
                                     `retry_times_by_overrated` int DEFAULT '3' COMMENT '连续限速超限放弃次数上限(防死循环)',
                                     `retry_times_by_partner` int DEFAULT '0' COMMENT '合作方异常重试次数',
                                     `retry_times_by_program` int DEFAULT '0' COMMENT '程序异常重试次数',
                                     `log_level` int DEFAULT '0' COMMENT '日志类型',
                                     `log_limit_size` int DEFAULT NULL COMMENT '日志长度限制',
                                     `stats_date` datetime DEFAULT NULL COMMENT '最后统计时间',
                                     `stats_run_num` int DEFAULT NULL COMMENT '统计运行次数',
                                     ...
                                     `alert_fail_data_rate` int DEFAULT NULL COMMENT '数据失败率',
                                     `alert_run_timeout` int DEFAULT NULL COMMENT '运行超时(毫秒)',
                                     `alert_wait_timeout` int DEFAULT NULL COMMENT '延迟超时(实际执行晚于runAt的平均毫秒)',
                                     `task_link_our` varchar(500) DEFAULT NULL COMMENT '我方联系信息',
                                     `task_link_mch` varchar(500) DEFAULT NULL COMMENT '商户联系信息',
                                     `create_date` datetime(3) DEFAULT NULL COMMENT '创建日期',
...
-- uw_task.task_delayer_stats definition

CREATE TABLE `task_delayer_stats` (
                                      `id` bigint NOT NULL COMMENT 'id',
                                      `task_id` bigint DEFAULT NULL COMMENT '任务配置id',
                                      `num_all` int DEFAULT NULL COMMENT '全部执行计数',
                                      `num_fail_program` int DEFAULT NULL COMMENT '程序错误计数',
                                      `num_fail_config` int DEFAULT NULL COMMENT '配置错误计数(连续限速超限放弃)',
                                      `num_fail_data` int DEFAULT NULL COMMENT '数据错误计数',
                                      `num_fail_partner` int DEFAULT NULL COMMENT '对方错误计数',
                                      `time_wait` int DEFAULT NULL COMMENT '实际延迟等待(runAt到consumeDate毫秒)',
                                      `time_run` int DEFAULT NULL COMMENT '运行时间(毫秒)',
...
CREATE TABLE `task_host_info` (
                                  `id` bigint NOT NULL COMMENT 'id',
                                  `host_ip` varchar(100) DEFAULT NULL COMMENT '主机注册IP',
                                  ...
                                  `task_project` varchar(100) DEFAULT NULL COMMENT '任务项目',
                                  `run_target` varchar(100) DEFAULT NULL COMMENT '运行目标',
                                  ...
                                  `delayer_num` int DEFAULT NULL COMMENT '延迟任务数量',
                                  `delayer_run_num` int DEFAULT NULL COMMENT '延迟任务统计运行次数',
                                  `delayer_fail_num` int DEFAULT NULL COMMENT '延迟任务统计运行失败次数',
                                  `delayer_run_time` bigint DEFAULT NULL COMMENT '延迟任务统计总时间毫秒数',
